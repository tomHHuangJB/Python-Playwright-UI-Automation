...
help:
	@printf "%s\n" \
		...
		"  make route-gap-report   Render the route coverage gap report" \
		"  make route-gap-report-json Render the route coverage gap report JSON" \
		"  make route-gap-report-csv  Render the route coverage gap report CSV" \
...
route-gap-report:
	@$(VENV_ACTIVATE) && python scripts/render_route_gap_report.py

route-gap-report-json:
	@$(VENV_ACTIVATE) && python scripts/render_route_gap_report.py --format json

route-gap-report-csv:
	@$(VENV_ACTIVATE) && python scripts/render_route_gap_report.py --format csv
...
ci-summary:
	...
	@$(VENV_ACTIVATE) && python scripts/render_route_gap_report.py --format json > /tmp/route-gap-report.json
	@$(VENV_ACTIVATE) && python scripts/render_ci_summary.py \
		...
		--route-gap-report-json /tmp/route-gap-report.json \
		...
		--route-gap-report-artifact local-route-gap-report \
